...
try
    ...
    p.subj_num = input('Subject number: ');
    p.task1_is_color = mod(p.subj_num,2);
    p.task1_is_scale = mod(p.subj_num,4) < 2;       
    ...
    %Instructions
    text = 'Your score will start from 0 now. You will see your accumulated score at the end of each block.\n\nOne final reminder to use the whole confidence scale, and to use the "Opt Out" option when you are less than 67% certain in your response!\n\nPress any key to begin with the experiment!';
    DrawFormattedText(window, text, 300, 'center', 255, p.wrapat);
    Screen('Flip',window);
    WaitSecs(1);
    KbWait;
    ...
    for run_number=1:p.number_runs
        ...
        for block_number=1:p.number_blocks
            ...
            text_block = ['BLOCK ' number_block_string '  (out of 4)'];
            ...
            if run_number < p.number_runs || block_number < p.number_blocks
                ...
                text = ['Total points: ' num2str(p.total_points)];
                Screen('TextSize',window, 50);
                DrawFormattedText(window, text, 'center', 200, 255, p.wrapat);
                Screen('TextSize',window, 30);
